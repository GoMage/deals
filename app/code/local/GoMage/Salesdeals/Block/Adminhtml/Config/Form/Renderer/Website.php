...
class GoMage_Salesdeals_Block_Adminhtml_Config_Form_Renderer_Website extends Mage_Adminhtml_Block_System_Config_Form_Field
{
    ...
    protected function _getElementHtml(Varien_Data_Form_Element_Abstract $element)
    {
    	...
    	$info = Mage::helper('gomage_salesdeals')->ga();
    	...
    	foreach (Mage::app()->getWebsites() as $website) {
            ...
            $element->setValue($id);
            $element->setClass('gomage-salesdeals-available-sites');
        ...
        $html .= '
        	...
        	function updateGomageSalesdealsWebsites(){
        		
        		$("'.$idprefix.'_diasbled").disabled = "disabled";
        		
        		if($$(".gomage-salesdeals-available-sites:checked").length >= '.intval($info['c']).'){
    				$$(".gomage-salesdeals-available-sites").each(function(e){
    					if(!e.checked){
    						e.disabled = "disabled";
    					}
    				});
    				...
    				if($$(".gomage-salesdeals-available-sites:checked").length == 0){
        	...
        	$$(".gomage-salesdeals-available-sites").each(function(e){
        		e.observe("click", function(){
        			updateGomageSalesdealsWebsites();
        		});
        	});
